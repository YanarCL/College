...
main_menu_image = load_image('D:\AirHockey\images\FonForPlay.png')
mode_select_image = load_image('D:\AirHockey\images\FonForPlay.png')
...
def start_vs_computer_game():
    global bg_label, canvas, score_a, score_b, ball_speed_x, ball_speed_y, ball_is_moving, is_paused, game_over, game_mode, player_started_moving, ball_update_id, paddle_move_id
    
    if ball_update_id is not None:
        root.after_cancel(ball_update_id)
    if paddle_move_id is not None:
        root.after_cancel(paddle_move_id)
    ...
    ball_update_id = root.after(20, update_ball)
    paddle_move_id = root.after(20, move_paddles)
...
def restart_game():
    global score_a, score_b, ball_speed_x, ball_speed_y, ball_is_moving, is_paused, game_over, player_started_moving, ball_update_id, paddle_move_id
...
def update_ball():
    global ball_speed_x, ball_speed_y, score_a, score_b, game_over, ball_update_id
...
def return_to_game_menu():
    global is_paused, game_over, player_started_moving, ball_update_id, paddle_move_id

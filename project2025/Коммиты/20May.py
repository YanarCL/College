...
def check_ball_collision():
    ...
    # Функция для проверки залипания
    def check_stuck(paddle_coords):
        paddle_center_x = (paddle_coords[0] + paddle_coords[2]) / 2
        ball_center_x = (ball_coords[0] + ball_coords[2]) / 2
        # Если шайба явно внутри биты (сильное перекрытие)
        overlap = min(paddle_coords[2], ball_coords[2]) - max(paddle_coords[0], ball_coords[0])
        if overlap > BALL_RADIUS:
            # Выталкиваем шайбу в сторону от центра биты
            direction = 1 if ball_center_x < paddle_center_x else -1
            canvas.move(ball, direction * 10, 0)
...
def handle_collision(paddle_coords, ball_coords):
    ...
    # Сохраняем исходную скорость из настроек
    speed = game_settings['BALL_SPEED']
    
    # Особый случай: если шайба попадает точно в центр биты
    if length < PADDLE_RADIUS / 2:
        # Добавляем случайное небольшое отклонение
    ...
    # Устанавливаем новую скорость (такая же как в настройках)
    ball_speed_x = speed * dx
    ball_speed_y = speed * dy
...
def update_ball():
    ...
    if ball_is_moving:
        # Проверка на нулевую скорость (редкий случай)
        if ball_speed_x == 0 and ball_speed_y == 0:
            # Задаем небольшое случайное направление

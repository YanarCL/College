...
def open_settings():
    ...
    Label(settings_window, text="Настройки игры", font=('Arial', 16)).pack(pady=10)
    
    # Здесь можно добавить элементы управления настройками
    Label(settings_window, text="Скорость шайбы:").pack()
    speed_scale = Scale(settings_window, from_=5, to=20, orient=HORIZONTAL)
    speed_scale.set(BALL_SPEED)
    speed_scale.pack()
    
    Label(settings_window, text="Скорость биты:").pack()
    paddle_scale = Scale(settings_window, from_=3, to=15, orient=HORIZONTAL)
    paddle_scale.set(PADDLE_SPEED)
    paddle_scale.pack()
    
    def apply_settings():
        global BALL_SPEED, PADDLE_SPEED
        BALL_SPEED = speed_scale.get()
        PADDLE_SPEED = paddle_scale.get()
        settings_window.destroy()
    
    Button(settings_window, text="Применить", command=apply_settings).pack(pady=20)
...
def start_vs_computer_game():
    ...
    # Привязка клавиш (игрок управляет одной битой)
...
def move_computer_paddle():
    ...
    if ball_is_moving:
        ...
        if ball_speed_x > 0:  # Шайба движется вправо (к компьютеру)
            if ball_speed_y != 0:
                ...
                if paddle_center_y < predicted_y - 10 * COMPUTER_REACTION:
                    canvas.move(paddle_b, 0, PADDLE_SPEED * COMPUTER_REACTION)
            ...
            if paddle_center_y < center_y - 10:
                canvas.move(paddle_b, 0, PADDLE_SPEED * COMPUTER_REACTION * 0.5)
        ...
        center_y = HEIGHT / 2
        if paddle_center_y < center_y - 10:
            canvas.move(paddle_b, 0, PADDLE_SPEED * 0.5)
        elif paddle_center_y > center_y + 10:
            canvas.move(paddle_b, 0, -PADDLE_SPEED * 0.5)

...
photo = load_image('D:\wishpics_ru_9c61f87a9f2f66779f76a91e0c27c376.jpg')
...
def change_image():
    new_photo = load_image('d:\Stadion.jpg')
    if new_photo:
        ...
        settings_btn.place_forget()  # Скрываем кнопку "Настройки"
...
def return_to_main_menu():
    ...
    settings_btn.place(relx=0.5, rely=0.575, anchor='center')  # Показываем кнопку "Настройки"
...
def open_settings():
    ...
    settings_window.geometry('500x300')  # Фиксированный размер окна настроек
    settings_window.resizable(False, False)  # Запретить изменение размера окна
    ...
    label = Label(settings_window, text=settings_text, justify="left", padx=10, pady=10, font=('Arial', 12))
    label.pack()

    close_button = Button(settings_window, text='Закрыть', command=settings_window.destroy, font=('Arial', 12))
    close_button.pack(pady=10)
...
# Кнопка "Настройки"
settings_btn = Button(root, text='Настройки', width=35, height=2, bg='#080675', fg='#AFD7FF', font=('impact', 20, 'bold'), command=open_settings)
settings_btn.place(relx=0.5, rely=0.65, anchor='center')  # Размещаем кнопку "Настройки" между "Начать играть" и "Выход"
...
exit_btn_main.place(relx=0.5, rely=0.8, anchor='center')  # Размещаем кнопку выхода под кнопкой "Настройки"

...
is_paused = False
game_over = False
game_mode = "player"  # "player" или "computer"
keys_pressed = {
...
def open_settings():
    ...
    # Очки для победы
    Label(settings_window, text="Очки для победы:", font=('Arial', 12)).pack(pady=5)
    win_score_entry = Entry(settings_window, font=('Arial', 12), justify='center')
    win_score_entry.insert(0, str(WIN_SCORE))
    win_score_entry.pack(pady=5)
    
    # Скорость бит
    Label(settings_window, text="Скорость бит:", font=('Arial', 12)).pack(pady=5)
    paddle_speed_entry = Entry(settings_window, font=('Arial', 12), justify='center')
    paddle_speed_entry.insert(0, str(PADDLE_SPEED))
    paddle_speed_entry.pack(pady=5)
    
    # Скорость шайбы
    Label(settings_window, text="Скорость шайбы:", font=('Arial', 12)).pack(pady=5)
    ball_speed_entry = Entry(settings_window, font=('Arial', 12), justify='center')
    ball_speed_entry.insert(0, str(BALL_SPEED))
    ball_speed_entry.pack(pady=5)
    
    def save_settings():
        global WIN_SCORE, PADDLE_SPEED, BALL_SPEED
        try:
            WIN_SCORE = max(1, int(win_score_entry.get()))
            PADDLE_SPEED = max(1, int(paddle_speed_entry.get()))
            BALL_SPEED = max(1, int(ball_speed_entry.get()))
            settings_window.destroy()
        except ValueError:
            Label(settings_window, text="Введите корректные числа!", fg='red').pack()
...
def return_to_main_menu():
    ...
    settings_btn.place(relx=0.5, rely=0.55, anchor='center')
    exit_btn_main.place(relx=0.5, rely=0.7, anchor='center')
...
def move_computer_paddle():
    """ИИ для управления компьютерной битой"""
    ...
    # Если шайба движется к компьютеру
    if ball_speed_x > 0:
        # Двигаем биту к шайбе
        if ball_center_y < paddle_center_y and paddle_coords[1] > 0:
            canvas.move(paddle_b, 0, -PADDLE_SPEED)
        elif ball_center_y > paddle_center_y and paddle_coords[3] < HEIGHT:
            canvas.move(paddle_b, 0, PADDLE_SPEED)
    else:
        # Возвращаем биту в центр
        if paddle_center_y < HEIGHT / 2:
            canvas.move(paddle_b, 0, PADDLE_SPEED)
        elif paddle_center_y > HEIGHT / 2:
            canvas.move(paddle_b, 0, -PADDLE_SPEED)

def start_vs_computer_game():
    global game_mode
    game_mode = "computer"
    start_vs_player_game()

def start_vs_player_game():
    global bg_label, canvas, score_a, score_b, ball_speed_x, ball_speed_y, ball_is_moving, is_paused, game_over
...
def create_pause_menu():
    ...
    Button(pause_menu, text="В меню", font=("Arial", 24), 
...
def move_paddles():
    if not is_paused and not game_over:
        ...
        # Если игра против компьютера - управление компьютером
        if game_mode == "computer":
            move_computer_paddle()
        else:
            # Движение биты второго игрока (Стрелки)
...
def handle_collision(paddle_coords, ball_coords):
    ...
    ball_speed_x = current_speed * dx
    ball_speed_y = current_speed * dy
...
def end_game():
    ...
    Button(end_menu, text="В меню", font=("Arial", 24), 
